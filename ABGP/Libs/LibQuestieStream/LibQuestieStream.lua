-- small binary stream lib from questie (by aero)
-- code is a bit of a mess right now. Next version will include optimization
-- to string reading (ReadStringBySubstring)

local QuestieStreamLib = LibStub:NewLibrary("LibQuestieStream", 1)
...
function QuestieStreamLib:SetMode(mode)
    if mode then
        self._mode = mode
        if mode == "raw" then
            self.ReadByte = QuestieStreamLib._ReadByte_raw
            ...
            self._WriteByte = QuestieStreamLib._writeByte
        elseif mode == "1short" then
            self.ReadByte = QuestieStreamLib._ReadByte_1short
            self._WriteByte = QuestieStreamLib._WriteByte_1short
        else
            self.ReadByte = QuestieStreamLib._ReadByte_b89
            self._WriteByte = QuestieStreamLib._WriteByte_b89
        end
    else
        self.ReadByte = QuestieStreamLib._ReadByte_b89
        self._WriteByte = QuestieStreamLib._WriteByte_b89
